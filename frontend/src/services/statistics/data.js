...
class Data {
  ...
  unfoldHorizontally(columnIndex, columnsToUnfold) {
    if (!this.columnExists(columnIndex)) return;
    const unfoldedColumn = this.getColumn(columnIndex);

    // Fold the column if it is already unfolded
    if (unfoldedColumn.unfoldedVertically) {
      // Remove the columns that are generated by the unfolding
      this.columns = this.columns.filter((column) => column.parentColumnIndex !== columnIndex);

      unfoldedColumn.unfoldedVertically = false;
      return;
    }
    ...
    unfoldedColumn.unfoldedVertically = true;
  }
  resetUnfoldedColumns() {
    ...
    this.columns.forEach((column) => {
      ...
      if (column.parentColumnIndex !== null) {
        ...
        if (!parentColumn.unfoldedVertically) {
