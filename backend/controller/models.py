...
def get_models(projectId):
    ...
    for model in os.listdir(dataPath + projectId + '/models/'):
        with open(dataPath + projectId + '/models/' + model + '/info.json') as json_file:
...
def add_results_dict(projectId, modelId, data):
    ...
    if not debiaiUtils.modelExist(projectId, modelId):
        return "Dataset '" + modelId + "' in project : '" + \
    ...
    if result_structure is None:
        return "The project expected results need to be specified before adding results", 403
    ...
    project_block_structure = debiaiUtils.getProjectblockLevelInfo(
        projectId)
    ...
    for expected_result in result_structure:
        if expected_result["name"] not in expected_results_order:
            return "The expected result '" + expected_result["name"] + "' is missing from the expected_results_order Array", 403
    ...
    for given_expected_result in expected_results_order:
        ...
        if not result_expected:
            return "The given expected result '" + given_expected_result + "' is not an expected result", 403

    #  Check if all blocks referenced in the result tree exists
    resultsToAdd = {}

    for blockKey in tree:
        ok, msg = __check_blocks_of_tree_exists(projectId, result_structure, giv_exp_res,
                                                tree[blockKey], 0, sampleIndex, blockKey, resultsToAdd)
        if not ok:
            print(msg)
            return msg, 403

    # The given tree is complient, let's add the results
    newResults = utils.addToJsonFIle(dataPath + projectId + "/models/" +
                                     modelId + "/results.json", resultsToAdd)

    utils.addToJsonFIle(dataPath + projectId + "/models/" +
                        modelId + "/info.json", {"nbResults": len(newResults),
                                                 "updateDate": utils.timeNow()})
    debiaiUtils.updateProject(projectId)
    return 200


def __check_blocks_of_tree_exists(projectId: str, result_structure: list, giv_exp_res: dict, block: dict, level: int, sampleIndex: int, path: str, resultsToAdd: dict):

    # Check block exist in the data
    blockInfo = debiaiUtils.findBlockInfo(projectId, path)
    if not blockInfo:
        return False, "Error while adding the results : block '" + path + "' doesn't exist"
    ...
    if level == sampleIndex:
        ...
        if len(block) != len(giv_exp_res):
            raise ValueError("in : " + path + ", " +
                             str(len(block)) + " value where given but " + str(len(giv_exp_res)) + "where expected")
    ...
    for subBlockKey in block:
        ok, msg = __check_blocks_of_tree_exists(
            projectId, result_structure, giv_exp_res, block[subBlockKey], level + 1, sampleIndex, path + "/" + str(subBlockKey), resultsToAdd)
        if not ok:
            return False, msg
