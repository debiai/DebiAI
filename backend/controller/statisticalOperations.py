...
def pearsonCorrelation(data):
    ...
    pval = df.corr(method=lambda x, y: pearsonr(x, y)[
                   1]) - np.eye(*rho.shape)  # calculate the p-value
    # return the number of *
    p = pval.applymap(lambda x: (
        len([i for t in [0.01, 0.05, 0.1] if x <= t])))
    ret = rho.values.tolist()
    for i in range(rho.shape[0]):
        for j in range(rho.shape[1]):
            ret[i][j] = [float(rho[i][j]), float(p[i][j])]
    return ret, 200  # pearson correlation matrix with the significance of the coefficient
...
def spearmanCorrelation(data):
    ...
    pval = df.corr(method=lambda x, y: spearmanr(x, y)[
                   1]) - np.eye(*rho.shape)  # calculate the p-value
...
def entropy_discrete_xy(x, y, base=2):
    ...
    """
    assert len(x) == len(
        y), "The two provided samples should be of the same length"
    xy = np.c_[x, y]
...
def discrete_mutual_information(x, y, base=2):
    ...
    """
    assert len(x) == len(
        y), "Il faut y avoir la même longueur pour les 2 variables"
    return entropy_discrete(x, base) + entropy_discrete(y, base) - entropy_discrete_xy(x, y, base)
...
def continuous_mutual_information(x, y, k=1, base=2):
    ...
    for ii in range(n):
        if epsilon[ii] <= 1e-10:
            nx[ii] = len(x_tree.query_ball_point(
                x_tree.data[ii], r=1e-9, p=np.inf)) - 1
            ny[ii] = len(y_tree.query_ball_point(
                y_tree.data[ii], r=1e-9, p=np.inf)) - 1
        else:
            nx[ii] = len(x_tree.query_ball_point(
                x_tree.data[ii], r=epsilon[ii] - 1e-9, p=np.inf)) - 1
            ny[ii] = len(y_tree.query_ball_point(
                y_tree.data[ii], r=epsilon[ii] - 1e-9, p=np.inf)) - 1

    mi = (digamma(k) - np.mean(digamma(nx + 1) + digamma(ny + 1)) + digamma(n)
          ) / np.log(base)  # version (1) in kraskov scientific paper
   ...
   k: the number of neighbours to consider
    ...
    for i in range(len(continuous)):
        for j in range(i, len(continuous)):
            continuous[i][j] = continuous[j][i] = normalise_function(normalise, continuous_mutual_information(list_continuous[i], list_continuous[j], k=k, base=base), continuous_mutual_information(
                list_continuous[i], list_continuous[i], k=k, base=base), continuous_mutual_information(list_continuous[j], list_continuous[j], k=k, base=base))
    return continuous
   ...
   k: the number of neighbours to consider
    ...
    for i in range(len(discrete)):
        for j in range(i, len(discrete)):
            discrete[i][j] = discrete[j][i] = normalise_function(normalise, discrete_mutual_information(
                list_discrete[i], list_discrete[j], base=base), entropy_discrete(list_discrete[i], base=base), entropy_discrete(list_discrete[j], base=base))
    return discrete


def mixed_iterate_function(list_continuous, list_discrete, base=10, k=3, normalise="none"):
    """
     Parameters:
    -----------
    list_continuous: list of list of the continuous variables
    ...
    for i in range(len(list_continuous)):
        for j in range(len(list_discrete)):
            mixed[i][j] = normalise_function(normalise, mixed_mutual_information(np.array(list_continuous[i]), np.array(list_discrete[j]), n_neighbors=k, base=base),
                                             continuous_mutual_information(list_continuous[i], list_continuous[i], k=k, base=base), entropy_discrete(list_discrete[j], base=base))
    ...
    discrete = discrete_iterate_function(
        list_discrete, base=base, normalise=normalise)
...
def mutualInformation(data):
    ...
    if "normalise" in data:
        ...
        if normalise not in acceptedNormalise:
            return "normalise need to be either 'max' or 'min' or 'square root' or 'mean' or 'none'", 403
    else:
        normalise = 'max'

    # Calculate mutual informations between features
    if list_continuous != [[]] and list_discrete == [[]]:
        return continuous_iterate_function(list_continuous, k=k, base=base, normalise=normalise)
    elif list_continuous == [[]] and list_discrete != [[]]:
        return discrete_iterate_function(list_discrete, base=base, normalise=normalise)
    elif list_continuous != [[]] and list_discrete != [[]]:
        return mixed_iterate_function(list_continuous, list_discrete, base=base, k=k, normalise=normalise)
    else:
...
def avgdigamma(points, dvec):
    ...
    for i in range(N):
        ...
        num_points = len(tree.query_ball_point(
            points[i], dist - 1e-15, p=float('inf')))
...
def mutualAndHigherInformation(data):
    ...
    hdmi = higherDimensionMutualInformation({
        "k": k,
        "base": base,
        "X": list_continuous + list_discrete
    })
    ...
    return {
        "higherDimensionMutualInformation": hdmi,
