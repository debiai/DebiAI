...
class DataProvider:
    ...
    def get_data_id_list(self, view, _from=None, _to=None):
        try:
            if _from is not None and _to is not None:
                url = self.url + "view/{}/dataIdList?from={}&to={}".format(
                    view, _from, _to
                )
            else:
            ...
            print(
                "Error getting data id list from {} on view {}".format(self.url, view)
            )
            return []

    def get_data(self, view, data_id_list):
        try:
            r = requests.post(self.url + "view/{}/data".format(view), json=data_id_list)
            return r.json()
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
            raise Exception(
    ...
    def getModelResults(self, view, modelId, samples: list) -> dict:
        try:
            r = requests.post(
                self.url + "view/{}/model/{}/results".format(view, modelId),
                json=samples,
            )
            return r.json()
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
    ...
    def get_selection_id_list(self, view, selectionId):
        ...
        try:
            r = requests.get(
                self.url
                + "view/{}/selection/{}/selectedDataIdList".format(view, selectionId)
            )
            return r.json()
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
            return None
    ...
    def get_selections_id_list(self, view, selectionIds, intersection):
        ...
        samples = set(self.get_selection_id_list(view, selectionIds[0]))
        ...
        for selectionId in selectionIds[1:]:
            if intersection:  # Selection intersection
                ...
                samples = samples.union(self.get_selection_id_list(view, selectionId))
...
def setup_data_prividers():
    ...
    if "DATA_PROVIDERS" in config and len(config["DATA_PROVIDERS"]) > 0:
        for provider_name in config["DATA_PROVIDERS"]:
            ...
            print(
                "Configuring data provider " + provider_name + " [" + provider_url + "]"
            )
            ...
            if not is_data_provider_up(provider_url):
                ...
                data_providers.append(DataProvider(provider_name, provider_url))
                    ...
                    debiai_models.append(
                        {
                            ...
                            "id": model["id"],
                            "nbResults": model["nbResults"]
                            if "nbResults" in model
                            else 0,
                            "creationDate": timeNow(),
                            "updateDate": timeNow(),
                            "metadata": {},
                ...
                projects.append(
                    {
                        ...
                        "name": data_provider_info[view]["name"]
                        if "name" in data_provider_info[view]
                        else view,
...
def get_data(projectId, sampleIds):
    project = getProjectById(projectId)
    data_provider = getDataproviderByname(project["dataProvider"])
    return {"dataMap": True, "data": data_provider.get_data(project["view"], sampleIds)}
